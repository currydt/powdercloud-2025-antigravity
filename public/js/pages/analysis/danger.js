...
filterCmp, powdercloudSvgRatingDanger;
...
function Controller(){
	...
	this.ratingChart_data = new Array();
	    ...
	    // Rating Chart Data - Start
            ...
            if (date != null) {
                if (record.data.danger_scale_alpine_code) {
                    this.ratingChart_data.push({category:0, date: date, ratingCode: record.data.danger_scale_alpine_code});
                }

                if (record.data.danger_scale_treeline_code) {
                    this.ratingChart_data.push({category:1, date: date, ratingCode: record.data.danger_scale_treeline_code});
                }

                if (record.data.danger_scale_below_treeline) {
                    this.ratingChart_data.push({category:2, date: date, ratingCode: record.data.danger_scale_below_treeline_code});
                }
            }
	...
	if (powdercloudSvgRatingDanger) {
            var svg = $('#svg_div').svg('get');	// Get svg
            powdercloudSvgRatingDanger.drawChart(svg, this.ratingChart_data, this.filterCmp.start_date, this.filterCmp.end_date);
        }

    }, this);
}

Controller.prototype.drawRatingDanger = function(){

    var categories = ["High Alpine", "Alpine", "Sub Alpine"];
    var days = [1, 2, 3, 4, 5, 6, 7]; // inital days (will be overwritten once we get real data)
    var title = "Danger Ratings";
    ...
    powdercloudSvgRatingDanger = new PowdercloudSvgRating(svgWidth, svgHeight, categories, days, title, 80);

    $('#svg_div').svg(); // Create svg
    var svg = $('#svg_div').svg('get');	// Get svg
    powdercloudSvgRatingDanger.drawChart(svg, null);
}
...
Ext.onReady(function(){
	...
	tbar: new Ext.Toolbar({
	    ...
	    {
                id       :'danger_scale_treeline',
                header   : 'Treeline',
                width    : 100,
                sortable : false,
                dataIndex: 'danger_scale_treeline_desc'
            },
	    {
                id       :'remark_treeline',
	    ...
	    {
                id       :'danger_scale_below_treeline',
                header   : 'Below Treeline',
                width    : 100,
                sortable : false,
                dataIndex: 'danger_scale_below_treeline_desc'
            },
	    {
	...
	items:[
		...
		items:[{
		    xtype:'container',
		    html:'<p>The following table displays the danger fields in textual format. You can select an individual column to resize it; change its position,  or to hide and show the column. You can sort by date and time in ascending or descending order.</p>'
		},
		    grid
		]
	    }),
		...
		items:[{
		    xtype:'container',
		    html:'<p>The following chart displays the danger ratings as a summarized chart. The latest recorded value for each day is presented.</p>'
		},
		    new Ext.Container({
			...
			listeners:{
			    afterrender:{
				scope: this,
				fn: controller.drawRatingDanger
			    }
			}
		    })
		]
