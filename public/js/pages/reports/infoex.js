...
Ext.Ajax.timeout = 120000; //2 mins
...
function Controller(){
	...
	snowpackStructureGrid.getSelectionModel().selectAll();
	...
	avalancheNarrativeGrid.getSelectionModel().selectAll();
        store.each(function (record){
	...
	avalancheGrid.getSelectionModel().selectAll();
	...
	fieldWeatherSummaryGrid.getSelectionModel().selectAll();
        store.each(function (record){
	...
	stabilityGrid.getSelectionModel().selectAll();
        store.each(function (record){
	...
	weatherGrid.getSelectionModel().selectAll();
        var _startDate = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0);
	var _endDate = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59, 999);
	...
	newsGrid.getSelectionModel().selectAll();
        store.each(function (record){
	...
	terrainUsageGrid.getSelectionModel().selectAll();
        store.each(function (record){
...
Controller.prototype.onDateChange = function(value){
	...
	grid.getStore().setBaseParam('date_time_start__range', today.format('Y-m-d 00:00')+'/'+today.format('Y-m-d 23:59'));
	grid.getStore().load();
    }, this);
}

Controller.prototype.onExportData = function(){
    var obs = this.getSelectedObservations();
    if(obs){
	window.open("/infoex/export-observation/?kobs=" + obs + "&cmd=download");
		/*
		Ext.Ajax.request({
		    method: 'POST',
		    url: '/infoex/export-observation/',
		    params:{
			cmd: 'download',
			kobs: obs
		    },
...
Controller.prototype.onSubmitData = function(){
    Ext.MessageBox.confirm('Confirm InfoEx Submit','Please confirm that the selected Observations below should be submitted.', function(btn){
	if(btn == "yes"){

	    var obs = this.getSelectedObservations();
	    if(obs && isOperationInfoEx){
		Ext.MessageBox.show({
		    msg: 'Submitting...',
...
Controller.prototype.onPrintData = function(){
    var obs = this.getSelectedObservations();
    if(obs){
	Ext.Ajax.request({
	    method: 'POST',
	    ...
	    failure: function(action, options) {
		Ext.Msg.show({
			title:'Error pritning Data',
			msg: 'Error in printing data.',
			buttons: Ext.Msg.OK,
			icon: Ext.MessageBox.WARNING
		});
	    }
...
Controller.prototype.getSelectedObservations = function(){
    if(storeLoadCounter == 0){
	var observations = [];
	Ext.each(this.allGrids, function(grid){
	    Ext.each(grid.getSelectionModel().getSelections(), function(record){
		observations.push(record.get('key'));
	    }, this);
	}, this);
	...
	Ext.Msg.show({
		title:'Data Loading',
		msg: 'Please wait for all the data to load before executing this action.',
		buttons: Ext.Msg.OK,
		icon: Ext.MessageBox.WARNING
	});
    }
} //TODO does not alway return a value...
...
Controller.prototype.drawCharts = function(){
    ...
    */
    ...
    // High Charts
    var startDate = powdercloudHighChartsUtil.generateViewableStartDateUTC( new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0));
    var endDate = powdercloudHighChartsUtil.generateViewableEndDateUTC( new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59, 999));
...
Ext.onReady(function(){
    ...
    */
    ...
    snowpackStructureStore = new ModeDaterangeFilterStore({
	    ...
	    dir: 'DESC',
            'date_time_start__range': today.format('Y-m-d 00:00')+'/'+today.format('Y-m-d 23:59')
        },
    ...
    */
    ...
    var avalancheSM = new Ext.grid.CheckboxSelectionModel();
...
//TODO
    ...
    var stabilitySM = new Ext.grid.CheckboxSelectionModel();
    ...
    var weatherSM = new Ext.grid.CheckboxSelectionModel();
...
// News
   ...
   var newsSM = new Ext.grid.CheckboxSelectionModel();
    ...
    */
    ...
    var terrainUsageSM = new Ext.grid.CheckboxSelectionModel();
    ...
    */
	...
	items:[
	    ...
	    // 	Need Operation Name to Left Border
	    // Need Date below name
            {
                xtype:'container',
                html:'<p>The following is a list of observations for the current day. Select fields and the required Infoex subset are presented.</p>'
            },
            {
                xtype:'container',
                html:'<p>You can manage these observations using the above toolbar. The export lets you maintain a local copy in the CAAML 3.0.3 format. The Print provides a formatted PDF of these data. The Submission Button sends these data to the InfoEx.</p>'
            },
            {
                xtype:'container',
                html:'<p>Upon successful submission, we recommend the user wait approximately 1 hour to let the InfoEx update and then check their submission.</p>'
            },
