...
export class PowdercloudSnowpackGrid extends LitElement {
    ...
    async _fetchData() {
        try {
            ...
            const response = await fetch('https://powdercrowd-api.web.app/api/observations_shim/snowpack?limit=100');
            ...
            if (result.success && result.rows) {
                ...
                const filtered = result.rows.filter(row => {
                    ...
                    // 3. Check Type (Snowpack)
                    // We check if 'snowpack_narrative' is present as a heuristic
                    let typeMatch = false;
                    if (row.snowpack_narrative) {
                        typeMatch = true;
                    }
                ...
                this._data = filtered.map(row => ({
                    ...
                    structure_desc: row.snowpack_narrative || '-',
                    test_result: row.test_result || '-', // Field might need adjustment based on actual data
                    observer_desc: row.observer_desc || '-'
                }));
            }
        } catch (e) {
            console.error('Error fetching snowpack data:', e);
        }
    }
    ...
    render() {
        ...
        return html`
            ...
            <dashboard-grid 
                title="Snowpack Observations (Sorcerer Lodge - Mar 2012)"
                .columns="${columns}" 
                .data="${this._data}"
                ?paginated="${true}"
                ._rowsPerPage="${10}"
...
customElements.define('powdercloud-snowpack-grid', PowdercloudSnowpackGrid);
