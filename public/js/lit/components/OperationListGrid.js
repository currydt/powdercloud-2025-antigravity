...
import '../AppModal.js';
import '../forms/OperationForm.js';

export class OperationListGrid extends LitElement {
    static properties = {
        _data: { state: true },
        _isMock: { state: true },
    ...
    _handleRowClick(e) {
        ...
        // Let's check DashboardGrid.js: this.dispatchEvent(new CustomEvent('row-click', { detail: row ...
        // Actually usually it's detail: { ...row } or just row.
        // Looking at DashboardGrid.js: _handleRowClick(row, index) ... this.dispatchEvent(new CustomEvent('row-click', { detail: row ... wait no, it doesn't emit row-click!
        // Wait, I need to check DashboardGrid.js again.
...
customElements.define('operation-list-grid', OperationListGrid);
